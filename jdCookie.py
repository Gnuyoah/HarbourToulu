...
def re_pin(r):
	try: return compile(r'pin=(.*?);wskey=.*?;').findall(r)[0] if 'wskey=' in r else compile(r'pt_pin=(.*?);pt_key=.*?;').findall(r)[0] if r[:6] == 'pt_pin' else compile(r'pt_key=.*?;pt_pin=(.*?);').findall(r)[0]
	except: print(r, '\nck格式不正确，请检查\n')
